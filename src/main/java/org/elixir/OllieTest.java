...
//		maltParser = new MaltParser(new File(MALT_PARSER_FILENAME).toURI().toURL(), nullOption);
	...
	public static void main(String args[]) throws MalformedURLException {
		...
		// initialize
		OllieTest ollieTest = new OllieTest();

		// extract from a single sentence.
		String sentence = "President Obama will meet with Congressional leaders on Friday, "
				+ "and House Republicans summoned lawmakers back for a Sunday session, in a "
				+ "last-ditch effort to avert a fiscal crisis brought on by automatic tax "
				+ "increases and spending cuts scheduled to hit next week.";

		Iterable<OllieExtractionInstance> extrs = ollieTest.extract(sentence);
